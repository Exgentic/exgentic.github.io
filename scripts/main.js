...
(function(){
  ...
  function buildGraph(rows){
    ...
    rows.forEach(r=>{
      ...
      if(!benchs[b]){
        ...
        benchs[b]={name:b,x:W/2+Math.cos(angle)*radius,y:H*.4+Math.sin(angle)*radius,vx:0,vy:0,r:8,type:'bench'};
      }
      edges.push({from:a,to:b,score,model:m});
    });
    Object.values(agents).forEach(a=>{
      const avg=a.scores.reduce((x,y)=>x+y,0)/a.scores.length;
      a.r=Math.max(18,avg*55);
  ...
  function simulate(){
    ...
    // Weak gravity — center in middle of shorter canvas
  ...
  function draw(){
    ...
    benchNodes.forEach(n=>{
      ...
      ctx.fillStyle=benchText;ctx.font='500 15px Inter';ctx.textAlign='center';
    ...
    agentNodes.forEach(n=>{
      ...
      ctx.font='600 17px Inter';ctx.textAlign='center';
      ctx.fillText(n.name.replace(/_/g,' '),n.x,n.y+n.r+26);
      // Score
      ctx.fillStyle=scoreText;ctx.font='500 15px JetBrains Mono';
      ctx.fillText((n.avg*100).toFixed(1)+'%',n.x,n.y+6);
...
window.addEventListener('scroll',()=>{
  ...
  lastScrollY=y;
  // Fade scroll hint after scrolling
  const hint=document.getElementById('scrollHint');
  if(hint && y>100) hint.classList.add('hidden');
  else if(hint && y<50) hint.classList.remove('hidden');
});
...
(function(){
  ...
  if(preferLight) document.body.classList.add('light-mode');
