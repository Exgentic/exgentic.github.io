...
const MODEL_DISPLAY={'claude-opus-4.5':'Opus 4.5','gpt-5.2':'GPT-5.2','gemini-3-pro':'Gemini 3'};
...
function processData(rows,modelFilter){
  ...
  rows.forEach(r=>{
    ...
    if(!groups[key])groups[key]={agent:r.visible_agent_name,model:r.model_normalized,benchmarks:{},costs:{}};
...
function renderTable(data){
  ...
  body.innerHTML=sorted.map((row,i)=>{
    ...
    let html=`<tr>`;
    html+=`<td>${rankHtml}</td>`;
    html+=`<td class="agent-name">${name}</td>`;
...
// ===== INIT =====
// Determine base path: use data attribute on script tag or default to '/'
...
if(_lastUpdatedEl){fetch(_basePath+'results.csv.timestamp').then(r=>r.text()).then(t=>{_lastUpdatedEl.textContent='Last updated: '+t.trim()}).catch(()=>{});}
